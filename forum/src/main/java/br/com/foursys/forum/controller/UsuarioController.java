...
public class UsuarioController {
	...
	@GetMapping("/{id}")
	public DetalheDoUsuarioDto detalhar(@PathVariable Long id) {
		Usuario usuario = usuarioRepository.getOne(id);
		return new DetalheDoUsuarioDto(usuario);
	}

	@PutMapping("/{id}")
	@Transactional
	public ResponseEntity<UsuarioDto> atualizar(@PathVariable Long id, @RequestBody @Valid AtualizarUsuarioForm form) {
		Optional<Usuario> optional = usuarioReposit+ory.findById(id);
